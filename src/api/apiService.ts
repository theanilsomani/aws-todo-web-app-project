...
const API_URLS = {
    create: 'YOUR_createTaskFunction_URL',
    list:   'YOUR_listTasksFunction_URL',
    update: 'YOUR_updateTaskFunction_URL',
    delete: 'YOUR_deleteTaskFunction_URL',
};
...
async function getAuthToken(): Promise<string | null> {
    try {
        ...
        if (error.name === 'UserUnAuthenticatedException' || error.message?.includes('User is not authenticated')) {
...
async function makeRequest<T = any>( // Generic type T for expected response data
    ...
    const options: RequestInit = { // Use RequestInit type from lib.dom.d.ts
        method: method,
        headers: {
    ...
    } catch (networkError: any) { // Catch network errors specifically
    ...
    try {
        if (contentType && contentType.includes("application/json")) {
         ...
         if (!response.ok) {
            throw new Error(`HTTP error ${response.status} - Failed to parse response`);
         }
...
export const createTaskAPI = async (taskText: string): Promise<Task> => {
    ...
    const response = await makeRequest<CreateResponse>('POST', API_URLS.create, { taskText });
    return { // Construct a Task object from the response
...
export const updateTaskAPI = async (taskId: string, updates: TaskUpdatePayload): Promise<Task> => {
    return makeRequest<Task>('PUT', API_URLS.update, { taskId, ...updates });
};

// Takes taskId, returns a confirmation message object
export const deleteTaskAPI = async (taskId: string): Promise<{ message: string }> => {
    return makeRequest<{ message: string }>('DELETE', API_URLS.delete, { taskId });
};
